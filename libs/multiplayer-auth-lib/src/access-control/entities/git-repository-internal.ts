...
export class GitRepositoryInternal extends EntityBaseProjectLevel {
  getEntityType() {
    return RoleProjectPermissionEntity.GIT_REPOSITORY_INTERNAL
  }
  ...
  async hasContextResourceAccess(): Promise<boolean> {
    ...
    if (this.params.projectId) {
      ...
      gitRepository = await GitRepositoryModel.findGitRepositoryByIdAndAndWorkspace(
        this.params._id,
        this.params.workspaceId,
      )
    }
