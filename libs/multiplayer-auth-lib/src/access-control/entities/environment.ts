...
export class Environment extends EntityBaseProjectLevel {
  getEntityType() {
    return RoleProjectPermissionEntity.ENVIRONMENT
  }
  ...
  async hasContextResourceAccess(): Promise<boolean> {
    const environment = await EnvironmentModel.findEnvironmentByIdAndProjectAndWorkspace(
      this.params._id,
