...
interface FormDataContentProps {
  readOnly?: boolean
  content: Notebook.FormDataContentType
  onChange: (val: Notebook.FormDataContentType) => void
}

const FormDataContent = ({ readOnly, content, onChange }: FormDataContentProps) => {
  const getEmptyProperty = () => {
    return BlocknoteTemplates.getEmptyBodyProperty(Notebook.BodyType.FORM_DATA)
  }

  const setProperties = (properties: Notebook.FormDataContentType) => {
    onChange(properties)
  }
  ...
  const changeProperty = (index: number, payload: Partial<Record<keyof Notebook.FormDataContentItem, any>>) => {
    if (readOnly) return
        ...
        <tr>
          <th colSpan={4}>
            <div className="px-2 py-1 gap-2 flex items-center">
              <span className="flex-1 font-normal">Form Data</span>
              {!readOnly && (
                <>
                  {content.length > 1 && (
          ...
          <tr key={index}>
            <td>
              <div className="flex items-center">
                ...
                />
                {/* Temporally hide file related fields */}
                {/* <select
                  value={param.type}
                  readOnly={readOnly}
                  className="text-gray-600"
                  onChange={e => changeProperty(index, { type: e.target.value, value: '' })}
                >
                  <option value={Notebook.FormDataPropertyType.TEXT}>Text</option>
                  <option value={Notebook.FormDataPropertyType.FILE}>File</option>
                </select> */}
              </div>
            </td>
            <td>
              {param.type === Notebook.FormDataPropertyType.TEXT ? (
                <AutoCompleteInput
                  placeholder="Value"
                  readOnly={readOnly}
                ...
                <FileInput
                  readOnly={readOnly}
                  value={param.value as Notebook.FileType}
                  onChange={newVal => changeProperty(index, { value: newVal })}
                />
              )}
            </td>
            <td>
