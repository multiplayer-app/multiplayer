...
export const PREDEFINED_VARIABLES: PredefinedVariable[] = [
...

  {
    ...
    getValue: () => {
      const ip = Array.from({ length: 8 }, () => Math.floor(Math.random() * 65536).toString(16))
      return ip.join(':')
    },
  },

  {
    ...
    getValue: () => {
      const mac = Array.from({ length: 6 }, () => Math.floor(Math.random() * 256).toString(16))
      return mac.join(':')
    },
  },
...

  {
    ...
    getValue: () => {
      const semver = Array.from({ length: 3 }, () => Math.floor(Math.random() * 10))
      return semver.join('.')
    },
  },
  ...
  // Names
  {
    ...
    getValue: () => {
      const firstNames = ['Ethan', 'Chandler', 'Megane', 'John', 'Jane', 'Alice', 'Bob']
  ...
  // profession
  {
    ...
    getValue: () => {
      const jobAreas = ['Mobility', 'Intranet', 'Configuration', 'Development', 'Design', 'Testing']
  ...
  },
  {
    ...
    getValue: () => {
      const jobDescriptors = ['Forward', 'Corporate', 'Senior', 'Junior', 'Lead', 'Principal']
...
export const predefinedVars = PREDEFINED_VARIABLES.map(v => ({
  ...v,
  secret: false,
  description: v.description,
  value: 'Computed at request execution',
  source: Blocknote.SourceEnv.PREDEFINED,
}))
