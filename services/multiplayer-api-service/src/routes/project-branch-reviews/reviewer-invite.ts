...
import { UserLib, NotificationLib } from '../../lib'
...
const sendInviteForActiveWorkspaceUsers = async (
  ...
  }).catch(err => logger.error(err, 'Failed to send DESIGN_REVIEW_NOTIFICATION notification'))
}
...
const sendInviteForPendingWorkspaceUsers = async (
  ...
  }).catch(err => logger.error(err, 'Failed to send INVITE_AND_DESIGN_REVIEW_NOTIFICATION notification'))
}
...
export default async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (emails.length) {
      invitedWorkspaceUserIdsByEmails = await Promise.all(
        emails.map(email => UserLib.inviteUser(
