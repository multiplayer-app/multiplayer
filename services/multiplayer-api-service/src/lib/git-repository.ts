...
// Buffers the provider file stream into a string - used by in-process callers
// that just want the file's contents (unlike the download route, which pipes
// the stream straight to the HTTP response instead of buffering it).
export const getFileContentsByGitId = async (
  workspaceId: string,
  projectId: string,
  gitRef: IGitRef,
): Promise<string> => {
  const gitRepository = await fetchGitRepositoryByGitId(workspaceId, projectId, gitRef.repositoryId)
