  ...
  const entityCommits = await Promise.all(entitiesList.map(
    async ({ entityId, entityCommitChangeType, entityCommitId }, index) => {
      ...
      const createEntityCommitResponse = await request(app)
        .post(`${API_PREFIX}/version/workspaces/${workspaceId}/projects/${projectId}/branches/${projectBranchId}/entities/${entityId}/commits`)
      ...
      if (entityCommitChangeType !== EntityCommitChangeType.DELETE) {
        ...
        const updateEntityCommitResponse = await request(app)
          .patch(`${API_PREFIX}/version/workspaces/${workspaceId}/projects/${projectId}/branches/${projectBranchId}/entities/${entityId}/commits/${entityCommit._id}`)
          .set('Accept', 'application/json')
          .set('Cookie', [cookie])
          .expect('Content-Type', /application\/json/)
          .send({
...
export const createCommit = async (
  ...
  const { body: commit } = await request(app)
    .post(`/internal${API_PREFIX}/version/workspaces/${workspaceId}/projects/${projectId}/branches/${projectBranchId}/commits`)
    .set('Accept', 'application/json')
    .set('Cookie', [cookie])
    .expect('Content-Type', /application\/json/)
    .send({
