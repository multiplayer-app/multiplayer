...
describe('Branch', () => {
  ...
  test('Branch should have last commit meta', async () => {
    ...
    const entityAResponse = await request(app)
      .post(`${API_PREFIX}/version/workspaces/${workspace._id}/projects/${project._id}/branches/${newBranch._id}/entities`)
      .set('Accept', 'application/json')
      .set('Cookie', [cookie])
      .expect('Content-Type', /application\/json/)
      .send({
    ...
    const { body: updatedNewBranch } = await request(app)
      .get(`${API_PREFIX}/version/workspaces/${workspace._id}/projects/${project._id}/branches/${newBranch._id}`)
      .set('Accept', 'application/json')
      .set('Cookie', [cookie])
      .expect('Content-Type', /application\/json/)
      .expect(200)
