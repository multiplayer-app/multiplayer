...
describe('Entity', () => {
  ...
  test('Create new entity', async () => {
    ...
    const { body: entityData } = await request(app)
      .post(`${API_PREFIX}/version/workspaces/${workspace._id}/projects/${project._id}/branches/${defaultBranch._id}/entities`)
  ...
  test('EntityCommit update should fail without file upload', async () => {
    ...
    const { body: entityCommit } = await request(app)
      .post(`/internal${API_PREFIX}/version/workspaces/${workspace._id}/projects/${project._id}/branches/${defaultBranch._id}/entities/${entity.entityId}/commits`)
      .set('Accept', 'application/json')
      .set('Cookie', [cookie])
      .expect('Content-Type', /application\/json/)
      .send({
    ...
    await request(app)
      .patch(`/internal${API_PREFIX}/version/workspaces/${workspace._id}/projects/${project._id}/branches/${defaultBranch._id}/entities/${entity.entityId}/commits/${entityCommit._id}`)
      .set('Accept', 'application/json')
      .set('Cookie', [cookie])
      .send({
